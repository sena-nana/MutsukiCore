...
PLUGIN_ID = "mutsuki-codex-core"
RUNNER_ID = "mutsuki-codex-core.codex-runner"
TASK_KIND = "effect.codex.run"
RESULT_EVENT_KIND = "codex.strategy.result"
...
class CodexEffectRunner:
    ...
    def descriptor(self) -> RunnerDescriptor:
        return RunnerDescriptor(
            ...
            contract_surfaces=(f"runner:{RUNNER_ID}", f"task_kind:{TASK_KIND}"),
        )
    ...
    async def _step_one(self, _ctx: RunnerContext, task: Task) -> RunnerResult:
        if task.kind != TASK_KIND:
            return _failed_result(
                task.task_id,
                route="codex.task.kind",
                evidence={"reason": "unsupported_task_kind", "task_kind": task.kind},
            )
        try:
        ...
        return RunnerResult(
            ...
            events=(
                DomainEvent(
                    event_id=f"{task.task_id}:codex.strategy.result",
...
def _failed_result(
    ...
    *,
    ...
    return RunnerResult(
        ...
        events=(
            DomainEvent(
                event_id=f"{task_id}:codex.strategy.result",
                kind=RESULT_EVENT_KIND,
