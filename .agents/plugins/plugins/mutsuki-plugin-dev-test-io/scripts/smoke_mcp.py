...
async def main() -> int:
    ...
    try:
        ...
        assert initialize["result"]["serverInfo"]["name"] == "mutsuki-plugin-dev-test-io"
        ...
        echo = await tool_call(
            ...
            "run_command",
            {
                "command": [
                    sys.executable,
                    "-c",
                    "print('mutsuki-plugin-dev-test-io-ok')",
                ],
                "timeout_ms": 5000,
            },
        )
        assert echo["exit_code"] == 0
        assert "mutsuki-plugin-dev-test-io-ok" in echo["stdout"]
