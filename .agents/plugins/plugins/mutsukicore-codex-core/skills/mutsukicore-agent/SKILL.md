...
Use this skill when Codex is asked to act as the core decision engine for a MutsukiCore Agent.
...
- Treat Codex as a `StrategyBackend` for a MutsukiCore `AgentSpec`, not as a Rust core concept.
...
- If Codex wants to use a tool, return a structured decision that names the intended MutsukiCore Operation. Do not call host resources or external SDKs directly from the runtime core.
