...
`python/legacy-mutsukibot/`，仅作为 legacy / reference material。
...
9. **双协议分离**：外部 wire shape 只能在 host / reference plugin / legacy sidecar；不得渗入 `crates/mutsuki-runtime-core` 或 `contracts`。
...
- 跨 contracts / core / host / legacy Python 边界的改动，先确认契约位置和测试入口。
- 不做打补丁式修复；定位根因，在正确层级修正。
- 优先沿用现有命名、错误码和测试风格。
- 不覆盖用户或其他 Agent 的已有改动；工作前后用 `git status --short` 或定向 diff 确认范围。
- 需要长期记录的背景、取舍和未决问题写进 `plans/`。

## Rust / Legacy Python 边界

- Root Rust crates 不依赖 Python。
- `python/legacy-mutsukibot/` 是旧 Python 框架、扩展、测试、docs 和 examples 的保留位置。
...
- Python 3.13 + uv 仅用于 `python/legacy-mutsukibot/`。
