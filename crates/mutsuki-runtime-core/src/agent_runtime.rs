...
use crate::backend::{BackendEventSink, BackendPayload, OperationBackend, RuntimeBackend};
...
use crate::scheduler::RuntimeTickOutcome;
...
impl AgentRuntime {
    ...
    pub fn tick_once_and_drive<B: RuntimeBackend>(
        &mut self,
        agent_id: &str,
        backend: &mut B,
    ) -> RuntimeResult<RuntimeTickOutcome> {
        let strategy = self.tick_once(agent_id, backend)?;
        let operation = strategy
            .decision
            .as_ref()
            .and_then(|decision| decision.get("operation").and_then(Value::as_str))
            .map(str::to_string);
        let payload = strategy
            .decision
            .as_ref()
            .and_then(|decision| decision.get("payload").cloned())
            .unwrap_or(Value::Null);
        let operation = match operation {
            Some(operation) => Some(self.invoke_operation(agent_id, &operation, payload, backend)?),
            None => None,
        };
    ...
    pub fn invoke_operation<B: OperationBackend>(
        ...
        match backend.invoke(agent_id, &key, payload, self) {
            Ok(result) => {
                self.emit_agent(
                    RuntimeEventKind::Operation,
                    "operation.invoke",
    ...
    fn commit_plugin_registries<B: OperationBackend>(
        ...
        let registry_agent_id = self
            .agents
            .keys()
            .next()
            .map(String::as_str)
            .unwrap_or("runtime");
        let operations = backend.list_operations(registry_agent_id)?;
        let sources = backend.list_sources(registry_agent_id)?;
...
impl BackendEventSink for AgentRuntime {
