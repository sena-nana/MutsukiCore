...
fn host_runtime_resource_commands_use_injected_resource_provider() {
    let config = HostRuntimeConfig::default()
        .with_resource_provider(COMMAND_PROVIDER_ID, Arc::new(CommandResourceProvider));
    let runtime = host_with_echo_runner()
...
#[test]
fn host_runtime_routes_mixed_active_resource_providers_by_provider_id() {
    let provider_a = "mutsuki.host.resource-a";
    let provider_b = "mutsuki.host.resource-b";
    let mut host = RuntimeBootstrapper::new();
    host.register_loaded_plugin(mixed_resource_provider_plugin(
        "plugin-resource-a",
        provider_a,
        "mutsuki.host.resource-a.blob",
    ));
    host.register_loaded_plugin(mixed_resource_provider_plugin(
        "plugin-resource-b",
        provider_b,
        "mutsuki.host.resource-b.blob",
    ));
    let runtime = host
        .into_host_runtime(RuntimeProfile {
            profile_id: "mixed-resource-providers".into(),
            mode: RuntimeProfileMode::FullDev,
            enabled_plugins: vec!["plugin-resource-a".into(), "plugin-resource-b".into()],
    ...
    let HostRuntimeReply::ResourceCreated(blob_a) = runtime
        .dispatch(HostRuntimeCommand::CreateBlobResource {
        ...
        panic!("expected provider a resource");
    };
    let HostRuntimeReply::ResourceCreated(blob_b) = runtime
        .dispatch(HostRuntimeCommand::CreateBlobResource {
            provider_id: provider_b.into(),
            schema: "bytes.v1".into(),
            bytes: b"b".to_vec(),
        })
        .unwrap()
    else {
        panic!("expected provider b resource");
    };
    assert_eq!(blob_a.provider_id, provider_a);
    assert_eq!(blob_b.provider_id, provider_b);
    ...
    assert_eq!(
        runtime
            ...
            .unwrap(),
        HostRuntimeReply::ResourceBytes(provider_a.as_bytes().to_vec())
    );
    let HostRuntimeReply::PlanReceipt(export_receipt) = runtime
        .dispatch(HostRuntimeCommand::ExecuteExportPlan(Box::new(
            ExportPlan {
                plan_id: "export:b".into(),
                resource: blob_b,
                target: "inline_utf8".into(),
                args: json!(null),
            },
        ...
        panic!("expected provider b export receipt");
    };
    assert_eq!(export_receipt.output, json!(provider_b));

    let HostRuntimeReply::ResourceCreated(capability_a) = runtime
        .dispatch(HostRuntimeCommand::CreateCapabilityResource {
            provider_id: provider_a.into(),
            kind_id: "query".into(),
            schema: "query.v1".into(),
        })
        .unwrap()
    else {
        panic!("expected provider a capability");
    };
    let HostRuntimeReply::ResourceCreated(capability_b) = runtime
        .dispatch(HostRuntimeCommand::CreateCapabilityResource {
            provider_id: provider_b.into(),
            kind_id: "query".into(),
            schema: "query.v1".into(),
        ...
        panic!("expected provider b capability");
    };
    let command_a = CommandPlan {
        plan_id: "command:a".into(),
        capability: capability_a,
        operation: "query".into(),
        args: json!(null),
        idempotency_key: None,
    };
    let command_b = CommandPlan {
        plan_id: "command:b".into(),
    ...
    let HostRuntimeReply::PlanReceipt(command_receipt) = runtime
        ...
        panic!("expected provider b command receipt");
    };
    assert_eq!(command_receipt.output, json!({"provider_id": provider_b}));

    let batch_error = runtime
        .dispatch(HostRuntimeCommand::ExecuteCommandBatch(Box::new(
            CommandBatch {
                batch_id: "batch:mixed".into(),
                commands: vec![command_a.clone(), command_b.clone()],
                rollback_guarantee: false,
            },
        )))
        .unwrap_err();
    assert_eq!(batch_error.error().code, ERR_REGISTRY_UNAUTHORIZED);

    let saga_error = runtime
        .dispatch(HostRuntimeCommand::ExecuteSagaPlan(Box::new(SagaPlan {
            saga_id: "saga:mixed".into(),
            steps: vec![command_a],
            compensations: vec![command_b],
        })))
        .unwrap_err();
    assert_eq!(saga_error.error().code, ERR_REGISTRY_UNAUTHORIZED);
}

#[test]
fn host_runtime_rejects_created_descriptor_from_different_provider() {
    let config = HostRuntimeConfig::default()
        .with_resource_provider("mutsuki.host.mismatched", Arc::new(CommandResourceProvider));
    let runtime = host_with_echo_runner()
        .into_host_runtime_with_config(runtime_profile(), config)
        .unwrap();
...
fn mixed_resource_provider_plugin(
    plugin_id: &str,
    ...
    PluginBuilder::new(plugin_id)
        .resource_type_descriptor(ResourceTypeDescriptor {
            ...
            provider_id: provider_id.into(),
            operations: vec!["collect".into(), "export".into(), "query".into()],
            reload_policy: ResourceProviderReloadPolicy::NoLiveResources,
            compatibility: ResourceProviderCompatibility {
                schema_version: "1.0.0".into(),
                required_operations: vec!["collect".into(), "export".into(), "query".into()],
                preserves_resource_type_id: true,
                accepts_older_generations: false,
                lease_drain_required: false,
            },
        })
        .resource_provider_gateway(provider_id, Arc::new(MixedResourceProvider { provider_id }))
        .build()
}

struct MixedResourceProvider {
    provider_id: &'static str,
}

impl mutsuki_runtime_sdk::ResourcePlanGateway for MixedResourceProvider {
    fn collect_read_plan(&self, _plan: &ReadPlan) -> RuntimeResult<Vec<u8>> {
        Ok(self.provider_id.as_bytes().to_vec())
    }
    ...
    fn execute_command_plan(&self, plan: &CommandPlan) -> RuntimeResult<PlanReceipt> {
        Ok(PlanReceipt {
            ...
            descriptor_updates: Vec::new(),
            new_version: None,
            output: json!({"provider_id": self.provider_id}),
        })
    }
...
impl mutsuki_runtime_sdk::ResourceProviderGateway for MixedResourceProvider {
    fn create_blob_resource(&self, schema: &str, _bytes: Vec<u8>) -> RuntimeResult<ResourceRef> {
        Ok(mixed_resource_ref(
            self.provider_id,
            &format!("{}:blob", self.provider_id),
            "mixed_blob",
            schema,
    ...
    fn create_cow_state_resource(
        ...
        Err(unused_test_provider_method("create_cow_state_resource"))
    }
    ...
    fn create_capability_resource(
        ...
        Ok(mixed_resource_ref(
            self.provider_id,
            &format!("{}:capability", self.provider_id),
            kind_id,
            schema,
