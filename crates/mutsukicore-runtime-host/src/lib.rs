...
mod tests {
    ...
    fn codex_agent() -> AgentSpec {
        AgentSpec {
            ...
            strategy_id: "mutsukicore-codex-core".into(),
            side_effect_policy: SideEffectPolicy::ReadOnly,
        }
    }
    ...
    impl PythonJsonlBackendProcess {
        fn spawn(
            ...
            let script = repo_root
                ...
                .join("plugins")
                .join("plugins")
                .join("mutsukicore-codex-core")
                .join("scripts")
                .join("mutsukicore_codex_strategy_backend.py");
            assert!(
                script.is_file(),
    ...
    fn native_host_rejects_stale_operation_generation() {
        ...
        assert_eq!(
            err.error().code,
            mutsukicore_runtime_contracts::ERR_RUNTIME_BACKEND_GENERATION_MISMATCH
        );
        assert!(
            runtime
                .trace_spans()
    ...
    fn jsonl_capability_backend_decodes_strategy_and_writes_request() {
        ...
        assert_eq!(
            result.status,
            mutsukicore_runtime_contracts::StrategyResultStatus::WaitInput
        );
    ...
    fn jsonl_runtime_backend_smoke_drives_codex_python_process_roundtrip_and_failure() {
        ...
        assert_eq!(error.source, "mutsukicore-codex-core");
    ...
    fn jsonl_capability_backend_dispatches_resource_register_acquire_and_release() {
        let token = mutsukicore_runtime_contracts::LeaseToken {
