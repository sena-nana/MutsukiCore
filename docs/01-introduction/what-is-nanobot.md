...
**NanoBot 是一个 Agent 中心的 Bot 框架，给 Yume / mind-sim 提供运行核心，同时通过插件组合复刻传统 Bot 框架的能力。**
...
但 NanoBot 不是只服务 Yume。任何"消息进来 → 命令路由 → 调插件 → 回响应"的传统 bot 形态，都能用 EchoPlugin 那种插件写法实现。Agent 与 Plugin 解耦：核心域中立，业务全在插件里。
...
- 不想写类型注解的人 —— NanoBot 的 PluginMeta / Dependent 完全依赖类型注解工作
