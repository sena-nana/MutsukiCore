...
代码：[`_build_command_spec`](../../nanobot/core/plugin.py#L170-L274) 在 [nanobot/core/plugin.py](../../nanobot/core/plugin.py)。
...
NanoBot 的做法是：**只让作者写一种东西**——一个带类型注解和 Google-style docstring 的 async 函数——schema 由元类自动从签名 + docstring + `Annotated[..., Arg(...)]` 合成。
...
具体合并代码在 [plugin.py:178-249](../../nanobot/core/plugin.py#L178-L249)。

### 描述

[plugin.py:182-186](../../nanobot/core/plugin.py#L182-L186)：
...
[plugin.py:188-191](../../nanobot/core/plugin.py#L188-L191)：
...
如果某参数 docstring 里没写描述，但 `Annotated[..., Arg(desc="...")]` 里写了，会作为 fallback 填进去（[plugin.py:243-245](../../nanobot/core/plugin.py#L243-L245)）。设计意图见 [contracts.plugin.Arg 注释](../../nanobot/contracts/plugin.py#L17-L24) —— **描述应该来自 docstring，`Arg` 主要承载约束**。

### 类型 → JSON Schema 类型

[plugin.py:145-167](../../nanobot/core/plugin.py#L145-L167) 的 `_json_type_for`：
...
完整代码在 [plugin.py:225-243](../../nanobot/core/plugin.py#L225-L243)。

### 哪些参数会被略过

[plugin.py:209-222](../../nanobot/core/plugin.py#L209-L222) 决定：以下两类参数**不**进 schema —— 它们由框架注入，不该让外部调用方看到：
...
[plugin.py:251-252](../../nanobot/core/plugin.py#L251-L252)：
...
如果函数声明了返回类型，`return_schema` 会按同样规则生成（[plugin.py:260-262](../../nanobot/core/plugin.py#L260-L262)）。当前 v0.1 不做强制校验 —— scheduler 直接 `str(result)` 后塞进出站消息。

### CommandSpec 的形态

`CommandSpec`（[contracts/plugin.py:90-104](../../nanobot/contracts/plugin.py#L90-L104)）是 `Contract`（即 `msgspec.Struct`）：
...
- **`func_qualname` 用于错误归因**，比如 trace 里出现 `plugin.nanobot-echo.echo`，方便回溯到源码 —— 不要为了 LLM 友好就给函数起特殊名字。
- **schema 与人类命令解析共用同一份**。Scheduler 用 `parameters_schema` 的 properties 顺序把 shell-style 位置参数对齐到 kwargs（[scheduler.py:152-155](../../nanobot/runtime/scheduler.py#L152-L155)），并按 `type` 做粗粒度强转（int / float / bool）。所以**参数声明顺序 = 命令位置参数顺序**。
