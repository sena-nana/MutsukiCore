...
- 协议：[nanobot/contracts/refpayload.py](../../nanobot/contracts/refpayload.py)
- 默认实现：[nanobot/core/handle.py](../../nanobot/core/handle.py)
...
NanoBot 的方案：把这种对象的"字段位置"用 `RefPayload[T]` 标记。codec / trace / audit 看到这个字段时，知道**只能落 descriptor，不能落 handle 本身**。运行时仍然用 handle 直接共享对象 —— 但在跨进程边界、序列化时会被显式拒绝。
...
[contracts/refpayload.py:49-82](../../nanobot/contracts/refpayload.py#L49-L82)：
...
[contracts/refpayload.py:35-46](../../nanobot/contracts/refpayload.py#L35-L46)：
...
[handle.py:56-134](../../nanobot/core/handle.py#L56-L134)。关键不变量：

1. **构造时 refcount = 1**（[handle.py:75](../../nanobot/core/handle.py#L75)）—— 这一份代表"调用者持有"
2. **第一次 `acquire` / `borrow` 之前必须 `attach_to(scope)`**（[handle.py:91-99](../../nanobot/core/handle.py#L91-L99) 的 `_check_attached`）—— 未 attach 即抛 `HandleNotAttachedError`，对应 `Errs.HANDLE_LEAK`
...
`borrow()` 是上下文管理器糖（[handle.py:125-131](../../nanobot/core/handle.py#L125-L131)）：
...
`attach_to(scope)` 把 handle 登记到 scope 的 handle 列表（[scope.py:87-89](../../nanobot/core/scope.py#L87-L89)）。scope.close 流程（[scope.py:127-143](../../nanobot/core/scope.py#L127-L143)）：
...
[handle.py:42-53](../../nanobot/core/handle.py#L42-L53) 的 `HandleImpl` 是 `Handle[T]` 的具体实现基类，子类通过 `__init_subclass__` 自动登记到 `HandleRegistry`：
...
[handle.py:137-157](../../nanobot/core/handle.py#L137-L157) 的工厂用于测试：
...
[contracts/refpayload.py:85-101](../../nanobot/contracts/refpayload.py#L85-L101)：
...
- **`release` 多调一次没事，调 acquire 之前 release 也没事**——已 released 的 handle 再 release 是 no-op（[handle.py:114-116](../../nanobot/core/handle.py#L114-L116)）。但 `acquire` released handle 会抛 `HandleUseAfterReleaseError`。
