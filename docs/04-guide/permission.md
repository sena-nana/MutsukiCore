...
- 规则 AST + 注册名：[mutsukibot/contracts/permission.py](../../mutsukibot/contracts/permission.py)
- 内置常量门面：[mutsukibot/contracts/permission_builtin.py](../../mutsukibot/contracts/permission_builtin.py)

## 解决什么问题

传统框架的 "permission decorator" 通常只支持简单字符串名（`@admin_only`）或硬编码角色检查。一旦组合需求出现 —— "owner 或 在 ops 频道" —— 就要套两层装饰器或者写一个新名字。MutsukiBot 借鉴 NoneBot 的 `Rule` / `Permission` 思路，把它合并成一种类型 `PermissionRule`，并支持 `&` / `|` 组合，AST 求值时严格保留布尔语义。

## 怎么工作

### PermissionRule：AST + check

[permission.py:40-84](../../mutsukibot/contracts/permission.py#L40-L84)：
...
`__and__` / `__or__` 在组合时把同类节点平展（[permission.py:72-84](../../mutsukibot/contracts/permission.py#L72-L84)），AST 保持浅；但**不**进一步合并 AND / OR —— `(a | b) & (c | d)` 严格按 (a OR b) AND (c OR d) 求值，而不是退化成四项 OR。

### PermissionName：可注册的命名规则

[permission.py:117-157](../../mutsukibot/contracts/permission.py#L117-L157)：
...
每个名字关联一个 checker。`PermissionName & PermissionRule` 与 `PermissionName & PermissionName` 都自动调用 `to_rule` 后再组合（[permission.py:145-157](../../mutsukibot/contracts/permission.py#L145-L157)）。

### 内置 Perms 门面

[permission_builtin.py](../../mutsukibot/contracts/permission_builtin.py) 注册两个：
...
[plugin.py:114-120](../../mutsukibot/core/plugin.py#L114-L120)：
...
```python
from mutsukibot import AgentContext
from mutsukibot.contracts.permission import PermissionName
...
- **`PermissionName.register` 是幂等 + owner 排他**。同 owner 多次注册同名只用第一次的 checker（`setdefault` 行为，[permission.py:138-139](../../mutsukibot/contracts/permission.py#L138-L139)）；不同 owner 抛 `PermissionConflictError`。
- **`perms_rule_id` 只是诊断标签**。它不参与求值，只在 `Errs.PERMISSION_DENIED` 的 evidence 里出现，方便定位是哪个命令拒了。它由 PluginMeta 自动设成 `f"{plugin_id}.{attr}"`（[plugin.py:367](../../mutsukibot/core/plugin.py#L367)）。
