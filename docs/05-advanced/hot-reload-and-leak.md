...
NanoBot 把"卸载插件后回到干净状态"做成可验证的契约：所有副作用都要登记到 `PluginScope`，scope.close 时统一回收，未释放的 handle 在 close 阶段被显式报告为泄漏（`Errs.HANDLE_LEAK`）。

代码：

- 卸载流程：[`PluginLoader.unload_from`](../../nanobot/core/loader.py#L122-L133)
- 泄漏检测：[`PluginScope.close`](../../nanobot/core/scope.py#L103-L167)
...
实现的难点不在卸载流程（按加载反序遍历就行），而在"怎么知道有没有清理干净"。NanoBot 的解法是把**清理验证**塞进 scope 关闭时：
...
`PluginLoader.unload_from`（[loader.py:122-133](../../nanobot/core/loader.py#L122-L133)）：

```python
while agent.plugins:
    entry = agent.plugins.pop()                       # 反序
    agent.detach_plugin(entry.plugin)                 # 从命令索引剔除
    try:
...
Agent 还有一个独立 scope（`_agent_scope`）专门给 lifespan 钩子用，由 `AgentScheduler.stop` 调用 `close_agent_scope` 关闭（[scheduler.py:65](../../nanobot/runtime/scheduler.py#L65)）。这条路径目前不参与泄漏统计 —— 但同一套 close 流程，所以行为一致。
...
要求：100 次反复装卸不抛 `HandleLeakError`、内存不持续增长、命令索引保持空。这是 v0.1 的硬门控。
...
- **重新装载同一个 Plugin 类时小心命令索引冲突**。`agent.attach_plugin` 用 `setdefault`，先来的不被覆盖；如果上一轮 detach 没干净（不该发生但 bug 会），新实例的命令永远不被路由。建议先 `unload_from` 完整跑完再 `load_into`。
...
- **`PluginRegistry` 在卸载流程末尾"unregister 实例 + register 类"**——这是为了让下一次 `load_into` 仍能找到这个类。它意味着同一进程里 PluginRegistry 不是真"卸了就没了" —— 卸载只是去掉运行实例，类定义仍可被加载。
