...
MutsukiBot 把"可扩展但要排斥拼写错误"的字符串集中处理 —— 三个子系统共用同一套 `RegisteredString` 骨架：

- `CapabilityName` —— 能力名
- `PermissionName` —— 权限名
- `ErrorCode` —— 错误码

代码：[mutsukibot/contracts/_registered.py](../../mutsukibot/contracts/_registered.py)。

## 解决什么问题

字符串作为标识符有两面性：扩展方便，但容易拼写错误。MutsukiBot 想要：
...
[_registered.py:19-103](../../mutsukibot/contracts/_registered.py#L19-L103) 关键片段：
...
`PermissionName` 因为有 checker 没法用 `bootstrap_facade`，自己手写 `_bootstrap()`（[permission_builtin.py:38-44](../../mutsukibot/contracts/permission_builtin.py#L38-L44)）。
