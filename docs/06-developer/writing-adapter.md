...
Adapter 把外部协议（CLI 输入 / WS 帧 / OneBot / HTTP webhook ……）翻译成 MutsukiBot 内部 `Message` 契约，反之亦然。它**不**含业务逻辑——业务靠插件实现。

代码：

- 基类：[mutsukibot/adapters/base.py](../../mutsukibot/adapters/base.py)
- 参考实现：[mutsukibot/adapters/inmemory.py](../../mutsukibot/adapters/inmemory.py)
...
[adapters/base.py:27-40](../../mutsukibot/adapters/base.py#L27-L40)：
...
[adapters/base.py:16-24](../../mutsukibot/adapters/base.py#L16-L24)：
...
[adapters/inmemory.py](../../mutsukibot/adapters/inmemory.py) 是测试驱动入口，最小的可工作 adapter：
...
`__init_subclass__` 自动登记到 `AdapterRegistry`（[registry.py:66](../../mutsukibot/core/registry.py#L66)）。需要时 `AdapterRegistry.get("inmemory")` 拿类。
