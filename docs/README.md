...
> 当前版本 **v0.1**。本套文档对应仓库内已经实现的代码，不是路线图。
> v0.2 之后才会落地的能力（codec 边界、CapabilityCounter 资源量化、跨进程隔离、运行时 lint 守卫等），见 [附录 · 未实现 / 路线图](appendix/roadmap-and-not-yet.md)。

NanoBot 是一个 **Agent 中心** 的 Bot 框架，给 Yume / mind-sim 提供运行核心，同时以插件组合的方式覆盖传统 Bot 框架的能力。文档章节按从浅到深排列；如果是第一次接触，按顺序读 01 → 02 → 03 即可在十分钟内跑通；如果要写自己的插件或排查机制问题，直接跳到 04。

## 章节导航

### [01. 介绍](01-introduction/)

- [什么是 NanoBot](01-introduction/what-is-nanobot.md) —— 一句话定位、分层、v0.1 已交付能力
...
- [跑通 Echo](03-quickstart/run-echo.md) —— 用 InMemoryAdapter 走完一遍消息闭环
...
- [写一个 Adapter](06-developer/writing-adapter.md)
...
- [`nanobot`](07-api/nanobot.md)
- [`nanobot.contracts`](07-api/contracts.md)
- [`nanobot.core`](07-api/core.md)
- [`nanobot.runtime`](07-api/runtime.md)
- [`nanobot.adapters`](07-api/adapters.md)
- [`nanobot.plugins`](07-api/plugins.md)
...
- 凡引用源码都用相对链接，例如 [agent.py:74](../nanobot/core/agent.py#L74)。
- 类、函数、命令名首次出现用反引号包起来；后续可省。
- 所有代码片段都从 `nanobot/` 现有实现里截取或仿写，可直接运行。
- 文档**不**记录 v0.2 才会有的特性 —— 任何带「计划中」「未来」字样的描述都集中在 [附录 · 未实现](appendix/roadmap-and-not-yet.md)。

## 如何贡献文档

文档与代码同 PR 改动。以下场景**必须**同步修改文档：

1. 新增 / 删除 / 重命名 `nanobot/__init__.py` 或 `nanobot.contracts/__init__.py` 的导出符号
