...
> 当前版本 **v0.3 MVP**。本套文档对应仓库内已经实现的代码，不是路线图。
> 尚未落地的扩展（跨进程隔离、类型化 Handle 注入、完整资源调度等），见 [附录 · 未实现 / 路线图](appendix/roadmap-and-not-yet.md)。
...
- 文档**不**记录 v0.3 才会有的特性 —— 任何带「计划中」「未来」字样的描述都集中在 [附录 · 未实现](appendix/roadmap-and-not-yet.md)。
