...
无内置调试面板。trace 已有 `JsonlTraceWriter` / `JsonlTraceReader` 的 JSONL 读写闭环，`mutsukibot.testing.replay_trace_spans(...)` 可校验重复 span、父链、时间区间与确定性排序；v0.4 的 `mutsukibot.testing.contract_kit` 还提供 `assert_trace_tree_closed`、`assert_cross_agent_trace_chain`、`assert_dispatcher_clean`。可视化 UI 仍在后续版本。
