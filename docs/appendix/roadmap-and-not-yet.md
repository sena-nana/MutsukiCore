...
当前 NanoBot 是**单进程**。`ServiceContainer` 解析的是进程内对象；`Bus` 派发只在进程内；Agent 与 plugin 实例都在同一 event loop。
...
[runtime/loop.py](../../nanobot/runtime/loop.py) 留了 `install_sync_point_guard(callback)` 占位钩子。完整的基于 `sys.settrace` 或 ruff 自定义规则的拦截在后续版本落地。
...
NanoBot 核心**不内置 LLM 调用**（hard rule #2）。LLM 必须是插件，而且是桥接插件 —— 包装外部 SDK，通过 `Caps.CALL_LLM` 声明，via service 提供给业务命令。
...
[plans/architecture.md](../../plans/architecture.md) 提到 NanoBot 是给 Yume / mind-sim 做运行核 —— 但 Yume 自身的实现路径将被解构为 NanoBot 之上的零散插件。
