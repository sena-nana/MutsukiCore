...
早期 Python 框架实现已经移动到 `python/legacy-mutsukibot/`。该目录是 legacy /
reference material，不再定义根级 runtime 主链。
...
- Legacy Python host 可作为可选 sidecar / adapter 存在，但不得被 Rust crates
  依赖。
...
- legacy Python 不被根级 Rust crates 依赖。
...
`python/legacy-mutsukibot/` 保留旧 Python framework、extension、tests、docs 与
examples，方便迁移和对照。它不再是当前主实现。
...
legacy reference 层通过 contracts、Operation、Source、Resource lease 组合表达。
