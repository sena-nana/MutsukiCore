...
cancel propagation 路径。该能力不承诺抢占已卡死 native step 或单连接同步 JSONL step；
worker health、独立 management channel 和强制隔离恢复留在 host supervision 后续扩展。
...
workflow 放入 contract surface；PluginHost 只校验 runner deployment 并启动 CoreRuntime。
