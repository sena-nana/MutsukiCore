...
- accepts。
- strategy_id。
- side_effect_policy：第一片仅锁定 `read_only` / `allow_external` 两个值。

### 必须内化

- participation 的语义。
- observer 默认只读，且即使匹配输入也不能成为 owner。
- explicit_helper 默认不接收外部输入，只能通过显式跨 Agent 调用参与。
...
- Agent 根据 profile 暴露 `effective_accepts` 与策略 id；无 profile 时保持既有 `accepts` 行为。
- AgentRegistry 在 owner 选择路径只接受 primary_candidate；广播路径仍保留旁路投递语义。
- Permission / Capability 后续根据 profile 限制副作用；当前第一片只保存声明。
