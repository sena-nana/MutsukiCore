...
| `AgentParticipation` | Agent 在外部输入流中的参与方式 | enum：`primary_candidate / observer / explicit_helper`（详见 §21） |
| `AgentProfile` | Agent 角色与策略配置层 | `profile_id`、`participation`、`accepts`、`strategy_id`、`side_effect_policy`（详见 §21） |
| `ExecutionStrategy` | Agent 内部下一步推进策略接口 | `strategy_id`、`supported_profiles`、`on_awake`、`on_input`、`on_stop`、`next_step`（详见 §21） |
| `StrategyResult` | strategy 单次推进结果 | `status`、`decision`、`emitted`、`error`（详见 §21） |
...
- Core 不内置应用后端 / CRUD / tool endpoint 能力名。非 IM 的外部后端、
  MCP、文件系统、浏览器等能力由 bridge / 领域插件通过
...
- transport / patch 版本切换 → Agent atomic_swap
...
- **建议命名**：`<plugin_id>.<method_name>`（来自 @command）或 `<source_namespace>.<op_name>`（来自 `dispatch.register_operation`）。如 `echo.echo`、`backend:default.notify`、`qq:bot1.send_msg`。
...
```text
SourceDescriptor:
  source_id: str                # agent-local 全限定，如 "qq:bot1" / "backend:default"
  kind: SourceKindName          # 注册式，如 SourceKinds.IM 或插件自定义 kind
...
外部后端、MCP、文件系统、浏览器等非 IM 来源必须由 bridge / 领域插件注册自有
kind：`SourceKindName.register("example.backend", declared_by="my-bridge-plugin")`。
Core 不内置 `TOOL` / `HYBRID` kind，避免把 Agent core 拉成应用后端抽象。
...
> **设计原则**：通用入站/出站载体，IM `Message` 是其特化。外部后端事件、
> Yume stimulus、MCP / 文件系统事件等由 bridge / 领域插件定义自己的
> Envelope 子类与 `payload_schema_id`。dispatcher 只按 `payload_schema_id`
> 与 `source` 路由，不内置应用后端 schema。
...
Envelope (Contract):
  ...
  payload_schema_id: str                  # 路由主 key（区分 Message / 领域 envelope）
...
ChannelRef (SourceRef):                   # IM 特化（v0.1 已有，v0.2 改为继承 SourceRef）
  ...
  # source_id 继承自 SourceRef，是 transport/source 的统一标识
```

### 16.3 Message 与领域 Envelope
...
BackendEvent (Envelope):                  # 示例：由 bridge / 领域插件定义，不属于 core
  schema_id: ClassVar[str] = "example.backend.event"
  event_type: str                         # 领域事件类型，如 "item.changed" / "fs.changed"
  payload: dict[str, Any]                 # 领域 payload；复杂场景用 RefPayload[T]
```

Core 不导出 `ToolEvent` / `ToolSourceRef`，也不提供应用后端 CRUD envelope。
真正的后端应持有业务数据，并通过 bridge plugin 把状态变化翻译为领域
Envelope；Agent 再通过 Operation 表达要采取的动作。
...
或 `ScopeName.register("example.backend", ..., rule=BySchemaPrefix("example.backend."))`。
...
- owner 选择路径（`rank_accepting` / `select_accepting`）额外筛掉非
  `AgentParticipation.primary_candidate` Agent；observer / explicit_helper
  即使匹配输入，也不能成为 owner。
- policy 只接收已匹配候选；默认策略是 `priority` 降序，平手按 `agent_id` 升序。
- `select_accepting(envelope)` 取排序后的第一个 owner 候选；
  `iter_accepting(envelope)` 保留广播语义，按 `Agent.effective_accepts` 投递
  primary / observer / helper 中显式匹配的 Agent。
...
## 21. AgentProfile / ExecutionStrategy 契约（agent-execution-boundary 第一片）

> **设计原则**：Agent 是身份、状态、生命周期、权限与调度边界的主体；
> `ExecutionStrategy` 只是在 Agent 授权边界内决定下一步如何推进。

### 21.1 AgentParticipation

```text
AgentParticipation:
  primary_candidate   # 可成为外部输入 owner
  observer            # 可收到旁路副本，默认只读，不成为 owner
  explicit_helper     # 不自动接收外部输入，只能被显式跨 Agent 调用
```
...
AgentProfile (Contract):
  schema_id: ClassVar[str] = "mutsukibot.agent_profile"
  schema_version: ClassVar[str] = "1.0.0"
...
`SideEffectPolicy` 第一片只锁定两个值：`read_only` 与 `allow_external`。当前实现
只保存声明，不在 dispatcher 中执行副作用拦截；后续由 permission / dispatcher
切片消费该字段。

`Agent.profile` 为空时保持兼容，等价于 `primary_candidate` + 既有
`Agent.accepts`。`Agent.effective_accepts` 有 profile 时取 `profile.accepts`，
否则取 `Agent.accepts`。

### 21.3 ExecutionStrategy
...
ExecutionStrategy (Protocol):
  ...
  next_step(ctx) -> Awaitable[StrategyResult]
```

约束：

- strategy 不拥有 Agent 生命周期，不修改 `agent_id`、owner、participation 或 accepts。
- strategy 调工具必须走 `ctx.dispatch.invoke(...)`。
- strategy 调其他 Agent 必须走 `ctx.dispatch.invoke_in_agent(...)`。
- strategy 不直接持有未挂入 `PluginScope` / `ResourceHost` 的 I/O 资源。

### 21.4 StrategyResult

```text
StrategyResult (Contract):
  schema_id: ClassVar[str] = "mutsukibot.strategy_result"
  schema_version: ClassVar[str] = "1.0.0"
