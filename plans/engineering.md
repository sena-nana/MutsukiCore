...
签名解析与 schema 合成实现：[nanobot/core/dependency.py](../nanobot/core/dependency.py)
（`Dependent` + `Param` ABC）与 [nanobot/core/plugin.py](../nanobot/core/plugin.py)
...
- **未申报即调用视为违规**。core 提供 [`check_capabilities`](../nanobot/core/capability_guard.py) 在调用入口检查。
...
- **Stub Handle 工厂** —— [`nanobot.core.handle.make_stub_handle(ref_id, *, kind, schema_id_target, schema_version_target, target, attributes)`](../nanobot/core/handle.py) 用于在没有真实后端（如 GPU）时生成可观测的假引用，便于上层插件单测。
...
- `pyrefly`：`project-includes = ["nanobot", "tests"]`、`python-version = "3.13"`。pyrefly 在元组变性、可调用类型协变上比 pyright 更严格，作为类型检查的第二把锁。
