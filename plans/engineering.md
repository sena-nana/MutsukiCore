...
`python/reference-mutsukicore/`，作为旧 Python 实现的参考与迁移层保留。
...
- **Python 3.13 + uv**：用于 `python/reference-mutsukicore/` 的旧实现与参考测试，
  以及 `python/mutsukicore-runtime-python/` 的新版 backend kit；二者都不是根级 Rust
  runtime 依赖。
...
MutsukiCore/
  ...
  Cargo.lock
  crates/
    mutsukicore-runtime-contracts/  # 纯协议对象与 ScopeRuleSpec 匹配
    mutsukicore-runtime-core/       # AgentRuntime / ResourceGate / backend traits
    mutsukicore-runtime-host/       # native in-memory host helper / smoke tests
  plans/
    roadmap.md
    architecture.md
    engineering.md
    ...
    version-reports/
  python/
    mutsukicore-runtime-python/    # 新版 Python backend kit：contracts mirror / host / resource / tests
    reference-mutsukicore/       # 旧 Python 框架、扩展、测试、docs、examples 的参考与迁移层
```

## 3. Rust Crate 边界

- `mutsukicore-runtime-contracts` 只定义可序列化纯数据结构：
  ...
  `ResourceRecord`。
- `mutsukicore-runtime-core` 实现 runtime mechanics：
...
- `mutsukicore-runtime-host` 提供 native Rust backend / host helper。它可以注册
...
Backend 可以是 native Rust host，也可以由 `python/mutsukicore-runtime-python` 提供的
...
改动 Python backend kit 时，从 `python/mutsukicore-runtime-python` 目录运行：
