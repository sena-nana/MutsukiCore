...
- **typer** —— CLI transport plugin（v0.1 后）
- **rich** —— 终端输出
- **pyyaml** —— 配置文件加载
...
**未通过 scope 注册的副作用即视为违规**。Lint 规则（v0.1 起）将检查直接调用全局 `asyncio.create_task`、`asyncio.get_event_loop()`、`bus.subscribe(...)` 不带 scope 参数等模式。
...
> **v0.1 → v0.2 变更**：原独立传输能力枚举已删除；能力名改注册式 `CapabilityName`。Core 只内置 IM 与通用运行能力；外部后端 / MCP / 文件系统 / 浏览器等非 IM 能力由 bridge 或领域插件自行注册。
...
- **外部兼容协议**（OneBot v11/v12、MCP、ChatCompletion 等）只能出现在 transport / bridge reference plugin 中。
- 不得渗透 `core` / `contracts`。
...
- **In-memory transport reference plugin** —— reference plugin 通过 dispatcher 注册 IM Source + Operation，无需真实平台即可驱动 Agent。
