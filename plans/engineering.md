...
- **ruff** + **pyright** —— lint 与类型检查（typeCheckingMode = "standard"）
...
> v0.0 仅产出 `AGENTS.md`、`README.md`、`plans/*`、`pyproject.toml`。`nanobot/` 与 `tests/` 在 v0.1 引入第一个可运行模块时再创建。
...
- `id`（kebab-case，如 `nanobot-yume-architecture`）
- `version`（SemVer）
...
插件命令通过装饰器声明，框架基于以下信息自动生成「人类可触发命令」与「Agent / LLM 可调用工具」的双重 manifest：

- 函数名 → 命令名 / 工具名
- docstring（首段为 description；可选 schema 块覆盖默认推断）
- 参数类型注解（含 `Annotated[..., Field(...)]`）→ 参数 schema
- 返回类型注解 → 工具返回 schema

两套调用路径共用同一个实现函数。**禁止维护两份**。
...
- 已知能力名（v0.0 草案）：`read_message` / `send_message` / `call_llm` / `persist` / `network_egress` / `spawn_agent` / `vram` / `kv_slot` / `train_quota` / `hold_ref` / `borrow_ref` / `produce_ref_stream`。后三项是通用 By-Ref 协议（[contracts.md §11](contracts.md#11-通用-by-ref-payload-协议)）的强制 capability。
- 量纲示例：`{"vram": "8GiB", "train_quota": "1 job"}`。
- **未申报即调用视为违规**。core 提供 `CapabilityGuard` 在调用入口检查。
...
- **内部契约**（NanoBot 自有）位于 `nanobot/contracts/`。
...
- **Stub Handle 工厂** —— 提供 `make_stub_ref(kind, attributes)` 用于在没有真实后端（如 GPU）时生成可观测的假引用，便于上层插件单测。
