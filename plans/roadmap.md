...
runner backend、stdio JSONL runner bridge、ValueRef/ResourceRef 资源管理测试实现。
...
- Rust core 覆盖：
  ...
  - runner cancel 走 management channel，dispose 进入 DisposeBag；TraceLog 记录
    ...
    contract_fingerprint，并将 runner.step span 绑定到 task trace_id、task id、
    task lease id、executor id 和 correlation id。
- Rust host 覆盖：
  ...
  - 当前 Host backend / plugin backend 裁剪是 Level 1：resolver 生成 active
    capability graph 和 contract surface，RuntimeBootstrapper 校验 runner deployment；尚未完成
    按 `RuntimeCapabilityGraph.active_*` 实例化、拒绝和监督 backend / bridge / codec /
    scheduler policy 的完整 registry。
...
- 继续加强 `MutsukiStdPlugins` 的 `mutsuki-plugin-resource-memory` provider，并补齐
  provider commit 后 descriptor registry 同步协议；host 侧 `ResourceProviderGateway`
  注入边界已覆盖资源创建和 plan 执行，core 只保留 descriptor、lease、generation
  与 occupancy 事实。
...
- 扩展真实 Host backend registry / supervision；当前 `HostExtensionDescriptor` 与
  `PluginBackend` 只固定边界和统一 client contract，不等于完成所有 backend 热替换执行器。
  验收时 Host 启动必须按 `RuntimeCapabilityGraph.active_*` 注册或拒绝 backend、bridge、
  codec、scheduler policy 和 workflow，裁剪后不得继续声明已裁剪实现可用。
