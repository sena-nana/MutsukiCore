...
- Rust contracts 覆盖 `Task`、`TaskStatus`、`ProtocolDescriptor`、`HandlerBinding`、
  `RunnerDescriptor`、`RunnerPurity`、`RunnerResult`、`StateDelta`、`EffectRequest`、
  `ResourceRef`、`ValueRef`、`RuntimeProfile`、`PluginManifest`、`RuntimeLoadPlan`、
...
- Rust core 覆盖：
  ...
  - `HandlerBindingRegistry` 从 load plan 物化，提供 protocol 到逻辑消费者绑定的
    查询索引；Core 不自动 fan-out。
  - 复杂编排由普通插件 runner 通过 `RunnerResult.tasks` 显式创建后续 task。
