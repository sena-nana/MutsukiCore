...
- Rust host 覆盖：
  ...
  - 当前 Host backend / plugin backend 裁剪是 Level 1：resolver 生成 active
    capability graph 和 contract surface，PluginHost 校验 runner deployment；尚未完成
...
- Rust SDK 覆盖：
  ...
  - 当前导出的 `TextBuffer`、`AstSnapshot`、`ProjectFacts`、`ModelOutputStream`、
    `DbPool` 是 resource descriptor marker 示例，后续应迁移到示例、测试 helper 或可选
    domain 包；核心 SDK 公共面应收敛为 `ResourceKind`、`TypedResourceHandle`、
    `ResourceClient` 和 plan 构造。
...
- 将 SDK 中的示例 resource marker 移出核心 SDK 公共默认导出，保留领域中立 plan /
  descriptor helper。
