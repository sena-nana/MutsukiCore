...
当前根级 workspace 只落地 runtime framework、Host / SDK 扩展边界和已接入的
...
前置能力。`dev-mock` 当前提供 deterministic echo / sleep / fail / random-fail /
...
当前已新增 `mutsuki-protocol-workflow`、`mutsuki-protocol-resource`、
...
- 继续加强 workspace 内 `mutsuki-plugin-resource-memory` provider；host 侧
