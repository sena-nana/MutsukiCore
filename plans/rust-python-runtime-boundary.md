...
- `crates/mutsuki-runtime-contracts`：Rust 纯数据协议与 `ScopeRuleSpec` 匹配。
- `crates/mutsuki-runtime-core`：Rust AgentRuntime MVP，覆盖生命周期、inbox
  调度、路由/选举、Operation metadata registry 与 trace bookkeeping。
- `mutsukibot.runtime.backend`：Python backend 协议、snapshot 与
  `PythonAgentBackend` adapter。
- `Dispatcher.list_operation_snapshots()` / `invoke_with_backend_key(...)`：
  暴露可序列化 handler 间接键，禁止跨边界泄漏 Python callable。

该实现仍是分层 MVP：Python Core 主链未被 Rust 替换，`PluginLoader`、真实
`Handle[T]`、Yume / LLM / transport 仍保留在 Python 层。
...
```text
OperationBackend:
  list_operations(agent_id) -> Result[tuple[OperationDescriptor, ...], Error]
...
- Operation 元数据刷新也是跨边界调用，失败必须结构化返回；Agent 启动事务不得在
  registry refresh 失败后进入可路由状态。
