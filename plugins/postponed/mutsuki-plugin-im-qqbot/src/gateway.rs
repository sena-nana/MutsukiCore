...
impl QqGatewayPump {
    ...
    fn next_task_id(&mut self) -> String {
        ...
        format!("mutsuki.im.qqbot.gateway.frame:{}", self.next_task_sequence)
    }
}
...
pub fn normalize_gateway_frame(frame: GatewayFrame) -> Result<Vec<DomainEvent>, String> {
    ...
    let kind = match event_type {
        ...
        "GROUP_MESSAGE_CREATE" | "GROUP_AT_MESSAGE_CREATE" => "mutsuki.im.qqbot.message.group",
        "C2C_MESSAGE_CREATE" => "mutsuki.im.qqbot.message.c2c",
        "INTERACTION_CREATE" => "mutsuki.im.qqbot.interaction",
        "MESSAGE_REACTION_ADD" | "MESSAGE_REACTION_REMOVE" => "mutsuki.im.qqbot.reaction",
        "FRIEND_ADD"
        | "FRIEND_DEL"
        | "C2C_MSG_REJECT"
        | "C2C_MSG_RECEIVE"
        ...
        | "GROUP_MSG_RECEIVE"
        | "GROUP_MEMBER_ADD"
        | "GROUP_MEMBER_REMOVE" => "mutsuki.im.qqbot.lifecycle",
        _ => "mutsuki.im.qqbot.gateway.unknown",
    };
    let event_id = frame
        .id
        .clone()
