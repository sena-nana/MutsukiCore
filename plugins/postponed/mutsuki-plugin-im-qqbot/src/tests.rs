...
fn manifest_declares_qqbot_runtime_surfaces() {
    ...
    assert!(
        manifest
            .provides
            .streams
            .contains(&"mutsuki.im.qqbot.gateway".into())
    );
    assert!(
        manifest
            .provides
            .subscriptions
            .contains(&"mutsuki.im.qqbot.gateway.events".into())
    );
    assert!(
        manifest
            .provides
            .timers
            .contains(&"mutsuki.im.qqbot.gateway.heartbeat".into())
    );
}
...
fn gateway_normalizer_maps_documented_events() {
    let cases = [
        ("READY", "mutsuki.im.qqbot.gateway.ready"),
        ("RESUMED", "mutsuki.im.qqbot.gateway.resumed"),
        ("GROUP_MESSAGE_CREATE", "mutsuki.im.qqbot.message.group"),
        ("GROUP_AT_MESSAGE_CREATE", "mutsuki.im.qqbot.message.group"),
        ("C2C_MESSAGE_CREATE", "mutsuki.im.qqbot.message.c2c"),
        ("INTERACTION_CREATE", "mutsuki.im.qqbot.interaction"),
        ("MESSAGE_REACTION_ADD", "mutsuki.im.qqbot.reaction"),
        ("GROUP_MEMBER_ADD", "mutsuki.im.qqbot.lifecycle"),
        ("UNEXPECTED", "mutsuki.im.qqbot.gateway.unknown"),
    ];
...
fn gateway_pump_tracks_session_and_reconnect_actions() {
    ...
    assert_eq!(
        pump.pop_action(),
        Some(GatewayAction::DispatchTask(
            "mutsuki.im.qqbot.gateway.frame:1".into()
        ))
    );
