...
def test_local_plugin_marketplace_entries_are_installable() -> None:
    ...
    assert {"mutsuki-codex-core", "mutsuki-test-io"} <= names
...
def test_test_io_mcp_config_points_to_existing_server_script() -> None:
    plugin_path = _repo_root() / ".agents" / "plugins" / "plugins" / "mutsuki-test-io"
    manifest = _read_json(plugin_path / ".codex-plugin" / "plugin.json")
    mcp_path = plugin_path / str(manifest["mcpServers"])
    mcp = _read_json(mcp_path)

    server = mcp["mcpServers"]["mutsuki-test-io"]
    assert server["command"] == "python"
    args = server["args"]
    assert isinstance(args, list)
    assert args == ["./scripts/mutsuki_test_io_mcp.py"]
    assert (plugin_path / args[0]).is_file()


def test_codex_core_manifest_exposes_strategy_skill_without_mcp_tools() -> None:
    plugin_path = _repo_root() / ".agents" / "plugins" / "plugins" / "mutsuki-codex-core"
    manifest = _read_json(plugin_path / ".codex-plugin" / "plugin.json")

    assert manifest["name"] == "mutsuki-codex-core"
    assert "mcpServers" not in manifest
    skill = plugin_path / "skills" / "mutsuki-core-agent" / "SKILL.md"
    script = plugin_path / "scripts" / "mutsuki_codex_strategy_backend.py"
