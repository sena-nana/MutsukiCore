...
预期输出（来自 [smoke.py](../../mutsuki/plugins/echo/smoke.py)）：

```
[smoke] agent smoke-agent phase=spawn
[smoke] loaded plugins: ['mutsuki-echo']
[smoke] phase=awake
[smoke] outbox -> 'echo: hello\n'
[smoke] phase=stop; trace at /tmp/mutsuki-echo-smoke.jsonl
```
...
`-e .` 是 editable install —— 你在 `mutsuki/` 下改代码立刻生效，不用重装。
...
**Q: pip 报 `ERROR: Package 'mutsuki' requires a different Python: 3.12.x not in '>=3.13'`**

A: Mutsuki 锁 Python 3.13+。装一个 3.13：
...
**Q: `mutsuki.plugins.echo.smoke` 报 `EntryPoint not found`**
