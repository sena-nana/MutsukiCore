...
本 quickstart 使用 `python/reference-mutsuki` 的旧 Python reference 插件链路。
...
| `EchoPlugin` | `@command` 自动注册 `mutsuki-echo.echo` Operation | [插件定义](../04-guide/plugin-definition.md) |
...
Python reference AgentScheduler._loop
  ...
  -> command text route: "echo" -> "mutsuki-echo.echo"
  -> Dispatcher.invoke(...)
  -> EchoPlugin.echo(...)
