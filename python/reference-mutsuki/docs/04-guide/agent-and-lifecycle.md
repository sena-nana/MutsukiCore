...
`Agent` 是 Mutsuki 里的**一等运行时实体** —— 不是会话、不是 LLM 调用、不是平台连接，而是带身份的常驻对象。当前 Rust 主链由 `AgentRuntime` 持有 lifecycle / routing / inbox tick 事实；本页代码示例描述 Python reference `Agent`，它有自己的 `agent_id`、事件总线、入站 / 出站队列、插件集合，以及由 Python reference `AgentScheduler` 驱动的 `asyncio` tick 循环。

代码：[mutsuki/core/agent.py](../../mutsuki/core/agent.py)。

## 解决什么问题

传统 bot 框架里，"bot" 通常是一个无状态的回调集合 + 一些会话级 state。Mutsuki 要承载 Yume / mind-sim 这类需要长时间运行、有内在状态、要主动行动的 agent，会话语义不够 —— 必须有一个明确的、可以被 spawn / awake / sleep / stop 的对象。
...
[agent.py:55-72](../../mutsuki/core/agent.py#L55-L72) 是个普通 dataclass：
...
`LifecyclePhase`（[lifecycle.py](../../mutsuki/contracts/lifecycle.py)）当前有三个运行阶段：
...
这让 control Agent、audit Agent、观察型 Agent 能在同一进程内同时接收一条 transport envelope。可运行验收入口见 [cross_agent_smoke.py](../../mutsuki/plugins/cross_agent_smoke.py)。
...
[agent.py:74-96](../../mutsuki/core/agent.py#L74-L96) 的 `make_context()` 创建上下文时，会用一个**懒初始化**的 `_agent_scope`：
...
为什么这么做：早期版本里，没有命令上下文时（如 `lifespan.fire`）我们直接借用第一个加载插件的 scope —— 结果那个插件被卸载时把 agent 的 lifespan 钩子上下文也带走了。现在 fallback scope 与任何插件解耦，由 Python reference `AgentScheduler.stop()` 显式 `await self.agent.close_agent_scope()` 关闭（[scheduler.py:65](../../mutsuki/runtime/scheduler.py#L65)）。
...
完整闭环在 [mutsuki/plugins/echo/smoke.py](../../mutsuki/plugins/echo/smoke.py)。
