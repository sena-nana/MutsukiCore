...
- 注册类型：[mutsuki/contracts/capability.py](../../mutsuki/contracts/capability.py)
- 内置常量门面：[mutsuki/contracts/capability_builtin.py](../../mutsuki/contracts/capability_builtin.py)
- Operation 调用守卫：[mutsuki/core/capability_guard.py](../../mutsuki/core/capability_guard.py)

## 解决什么问题

传统框架里"插件能不能调用网络"这种事情靠文档约定 —— 没人在加载时检查。结果生产环境出现 LLM 插件偷偷起本地 socket、调试插件持有 GPU 不释放等情形。

Mutsuki 的 hard rule #7：**未申报 capability 即调用视为违规**。所有"敏感能力"必须先在 manifest 里列出来；Operation 再用 `requires_capabilities=...` 进一步细粒度声明；dispatcher 在 invoke 前做"required ⊆ declared"检查。
...
[capability.py:25-34](../../mutsuki/contracts/capability.py#L25-L34)：
...
这让框架核心可以预先注册一组名字（[capability_builtin.py:31-44](../../mutsuki/contracts/capability_builtin.py#L31-L44)），第三方插件扩展自己的命名空间（如 `yume.vram`）—— 而 `CapabilityName("typo_here")` 这种打字错误会在构造时直接拒掉。

### 内置 Caps 门面

[capability_builtin.py:17-28](../../mutsuki/contracts/capability_builtin.py#L17-L28)：
...
[capability.py:37-46](../../mutsuki/contracts/capability.py#L37-L46)：
...
[capability_guard.py:20-36](../../mutsuki/core/capability_guard.py#L20-L36)：
...
按 [capability_builtin.py](../../mutsuki/contracts/capability_builtin.py) 的模式建一个门面类，让 IDE 能补全：
...
- **构造未注册的 `CapabilityName` 会立即抛错**。导入时序很重要：如果你在某模块顶层 `Caps.READ_MESSAGE`，必须保证 `mutsuki.contracts.capability_builtin` 已经被导入过（通常 `from mutsuki import Caps` 或 `from mutsuki.contracts import Caps` 就够了）。
