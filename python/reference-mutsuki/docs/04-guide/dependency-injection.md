...
代码：[mutsuki/core/dependency.py](../../mutsuki/core/dependency.py)。

## 解决什么问题

NoneBot 的 `Depends(...)` 系统证明了「按签名声明依赖」的工程价值：作者只关心"我需要什么"，框架负责"从哪里拿"。但 NoneBot 允许按参数名 fallback —— 一个名为 `bot` 的参数会自动注入当前 Bot 实例。Mutsuki 不这么做：参数必须**显式被某种 Param 认领**，否则在 parse 阶段就拒绝。原因是：

1. 按名 fallback 让重命名变 breaking change
2. 让"漏写注解"从 bug 变成"运气好"
3. 多人协作时增加心智负担

Mutsuki 的妥协：四类 Param + 严格 claim 流程，保留 NoneBot 心智，但加上类型门槛。
...
| Param | 认领条件 | 注入什么 | 代码 |
|---|---|---|---|
| `CtxParam` | 类型是 `AgentContext`（含子类） | 当前 ctx | [dependency.py:71-93](../../mutsuki/core/dependency.py#L71-L93) |
| `RefParam` | `Annotated[Handle[T], RefArg(kind="...")]` | 从 payload 或 `ResourceHost` 解析 typed handle，并校验 kind | [dependency.py](../../mutsuki/core/dependency.py) |
| `ServiceParam` | 默认值是 `Inject()` sentinel | `ctx.services.resolve(类型, name=inject.name)` | [dependency.py:128-146](../../mutsuki/core/dependency.py#L128-L146) |
| `ArgParam` | 以上都不认领，且不是 `Inject` 默认值 | `extras[name]`；缺则用默认值；再缺抛 `KeyError` | [dependency.py:96-125](../../mutsuki/core/dependency.py#L96-L125) |

claim 顺序由 `_DEFAULT_PARAMS` 决定（[dependency.py:169](../../mutsuki/core/dependency.py#L169)）：`CtxParam → RefParam → ServiceParam → ArgParam`。第一个返回非 None 的就赢，后续不再尝试。
...
[dependency.py:179-235](../../mutsuki/core/dependency.py#L179-L235) 的 `Dependent.parse(call)`：
...
[dependency.py:237-248](../../mutsuki/core/dependency.py#L237-L248)：
...
from mutsuki import AgentContext, Inject, Plugin, command
from mutsuki.contracts import Handle, RefArg, RefArgSource
