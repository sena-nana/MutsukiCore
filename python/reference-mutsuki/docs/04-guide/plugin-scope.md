...
代码：[mutsuki/core/scope.py](../../mutsuki/core/scope.py)。

## 解决什么问题

Mutsuki 的 hard rule #4：**无副作用热重载**。卸载插件后，必须没有它残留的 task / 订阅 / GPU 显存 / 连接。这是把 Yume / mind-sim 装载到长寿命 agent 里的前提 —— 出了 bug 要能整段换掉而不是重启进程。
...
[scope.py:65-89](../../mutsuki/core/scope.py#L65-L89)：
...
按 [ResourceKind](../../mutsuki/core/scope.py#L29-L41) 区分种类只是为了**诊断**——`HandleLeakError.evidence` 里要告诉你"是哪类资源没清理"。运行时所有清理函数共用一条反向释放循环（[scope.py:110-122](../../mutsuki/core/scope.py#L110-L122)）。

### close() 的执行顺序

[scope.py:103-167](../../mutsuki/core/scope.py#L103-L167)：
...
`RefCountedHandle` 构造时引用计数 = 1（[handle.py:75](../../mutsuki/core/handle.py#L75)）—— 这一份代表"调用方持有"。`attach_to(scope)` 不再 +1，只是把 handle 登记到 scope。scope.close 调用 `release()` 释放掉构造时的那一份；如果还有第三方在持有（比如某个 task 里 acquire 了没 release），refcount > 0，handle 仍 alive，就是泄漏。

### TransactionScope：commit / rollback 二选一

[scope.py:170-199](../../mutsuki/core/scope.py#L170-L199) 扩展了一种"补偿"语义：
...
- **scope 不能复用**。`close()` 后再 `add_subscription` 会抛 `RuntimeError("PluginScope(...) 已关闭")`（[scope.py:95-97](../../mutsuki/core/scope.py#L95-L97)）。
...
- **句柄一定要先 `attach_to(scope)` 再用**。`acquire` / `borrow` 会先调用 `_check_attached`，未 attach 直接抛 `HandleNotAttachedError`（[handle.py:91-99](../../mutsuki/core/handle.py#L91-L99)）—— 这是 contracts §11.2 的硬约束。
