...
Mutsuki 的因果链系统：dispatcher 调用、跨 Agent 调用、envelope consumer 和 ResourceHost 关键入口都会 emit `TraceSpan` 到事件总线；观察者订阅 `trace.span` 写到结构化 sink（JSONL、OTel、其他）。

代码：

- 上下文：[`TraceContext`](../../mutsuki/core/context.py#L25-L29)
- 契约：[`TraceSpan`](../../mutsuki/contracts/event.py#L19-L32)、[`Event`](../../mutsuki/contracts/event.py#L35-L48)、[`SpanStatus`](../../mutsuki/contracts/event.py#L14-L17)
- 默认观察者：[`JsonlTraceWriter`](../../mutsuki/observability/trace.py) / [`JsonlTraceReader`](../../mutsuki/observability/trace.py)
- 回放与契约断言：[`replay_trace_spans`](../../mutsuki/testing/trace_replay.py) / [`contract_kit`](../../mutsuki/testing/contract_kit.py)
...
[context.py:25-29](../../mutsuki/core/context.py#L25-L29)：
...
[scheduler.py](../../mutsuki/runtime/scheduler.py) 是 Python reference 层的兼容调度器，负责把 IM 文本解析成 Operation payload，并调用 dispatcher：
...
[contracts/event.py:19-32](../../mutsuki/contracts/event.py#L19-L32)：
...
[contracts/event.py:35-48](../../mutsuki/contracts/event.py#L35-L48) 的 `Event` 是更通用的"内部事件"包装：
...
[observability/trace.py](../../mutsuki/observability/trace.py) 的标准订阅者：

- `attach(bus)`：打开文件、订阅 `trace.span`
- `detach()`：unsubscribe + 关文件
- 写失败时不阻塞 publisher，转发到 bus 上的 `trace.write_failed` 事件（[trace.py:36-45](../../mutsuki/observability/trace.py#L36-L45)）

落盘格式（[trace.py](../../mutsuki/observability/trace.py)）：每行一个 JSON object，含 trace_id / span_id / parent_span_id / name / start / end / status / attributes。`JsonlTraceReader.read_all()` 按同构格式读回 `TraceSpan`；记录缺字段或字段类型错误时抛 `TraceReplayError`，其中 `error.code == Errs.TRACE_RECORD_INVALID`。

### Trace replay / contract kit

`mutsuki.testing.replay_trace_spans(...)` 不重放外部副作用，只验证已记录 span 的因果链，并返回稳定排序的 `TraceReplayFrame`：
...
v0.4 起，`mutsuki.testing.contract_kit` 在 replay helper 之上提供更高层断言：`assert_trace_tree_closed(...)`、`assert_cross_agent_trace_chain(...)`、`assert_dispatcher_clean(...)`。
...
```python
from mutsuki.observability import JsonlTraceReader
from mutsuki.testing.contract_kit import assert_trace_tree_closed
