...
Mutsuki 把时间、ID、随机数都做成**注入式**协议（`Clock` / `IdGen` / `RNG`）。生产环境用 `SystemClock` / `NanoIdGen` / `SeededRng`；测试里用 `ManualClock` / `DeterministicIdGen` / `SeededRng(seed=固定值)`。同一份输入 + 同一份 seed → 同一份 trace。

代码：

- Clock：[mutsuki/runtime/clock.py](../../mutsuki/runtime/clock.py)
- IdGen：[mutsuki/runtime/idgen.py](../../mutsuki/runtime/idgen.py)
- RNG：[mutsuki/runtime/rng.py](../../mutsuki/runtime/rng.py)
...
[clock.py:13-19](../../mutsuki/runtime/clock.py#L13-L19) 定义协议：
...
[clock.py:22-32](../../mutsuki/runtime/clock.py#L22-L32)：直接转 `time.time` / `time.monotonic` / `asyncio.sleep`。

#### ManualClock

[clock.py:39-96](../../mutsuki/runtime/clock.py#L39-L96)：墙钟与单调时间只在 `advance(seconds)` 调用时前进。`sleep(s)` 把一个 `(deadline, seq, asyncio.Event)` 三元组放进 min-heap，await event。`advance` 弹出所有 deadline ≤ 当前的 sleeper 并 set 它们的 event。
...
[clock.py:90-96](../../mutsuki/runtime/clock.py#L90-L96)：唤醒所有挂起的 sleeper（用于测试 teardown），返回被唤醒的数量。否则未唤醒的 task 在 event loop 关闭时会报 "Task was destroyed but it is pending"。

### IdGen

[idgen.py:15-19](../../mutsuki/runtime/idgen.py#L15-L19) 协议：
...
[idgen.py:22-36](../../mutsuki/runtime/idgen.py#L22-L36)：`<prefix>_<26 字符 Crockford-base32>`。前 10 位带时间因子用于粗排序，**不是安全令牌**（`secrets.token_bytes(16)` 只用一半，未充分熵）。

#### DeterministicIdGen（测试）

[idgen.py:39-48](../../mutsuki/runtime/idgen.py#L39-L48)：
...
[rng.py:9-15](../../mutsuki/runtime/rng.py#L9-L15) 协议：
...
[rng.py:18-31](../../mutsuki/runtime/rng.py#L18-L31)：薄包装 `random.Random(seed)`。生产 / 测试都用同一类型，区别只在 seed 来源。

### 注入路径

`Agent` 构造时由调用方传入（[agent.py:60-62](../../mutsuki/core/agent.py#L60-L62)）。Agent 不构造默认值——必须由调用方显式提供。`AgentContext` 把它们 by reference 传给命令（[context.py:38-40](../../mutsuki/core/context.py#L38-L40)）。
...
```python
from mutsuki.runtime import DeterministicIdGen, ManualClock, SeededRng
...
- **`ManualClock.sleep(0)` 直接返回**（[clock.py:64-65](../../mutsuki/runtime/clock.py#L64-L65)）——不进 heap。
