...
- [mutsuki/runtime/clock.py](../../mutsuki/runtime/clock.py)
- [mutsuki/runtime/idgen.py](../../mutsuki/runtime/idgen.py)
- [mutsuki/runtime/rng.py](../../mutsuki/runtime/rng.py)
...
`Agent` 构造时三参数为必传（[agent.py:60-62](../../mutsuki/core/agent.py#L60-L62)）：
...
之后传到每个 `AgentContext`（[context.py:38-40](../../mutsuki/core/context.py#L38-L40)），插件命令通过 `ctx.clock` / `ctx.id_gen` / `ctx.rng` 访问。
...
from mutsuki.runtime import SystemClock
