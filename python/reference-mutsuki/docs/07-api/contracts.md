# API · `mutsuki.contracts`

类型定义包；除三个内置门面（Caps / Perms / Errs）的注册副作用外，无运行时副作用。

来源：[mutsuki/contracts/__init__.py](../../mutsuki/contracts/__init__.py)。
...
[base.py](../../mutsuki/contracts/base.py)
...
[ids.py](../../mutsuki/contracts/ids.py)

`AgentId` / `TraceId` / `SpanId` / `RefId` / `MessageId` —— 全是 `NewType("...", str)`。pyright 会标记跨 ID 误赋值。

## lifecycle

[lifecycle.py](../../mutsuki/contracts/lifecycle.py)

`LifecyclePhase`：`SPAWN` / `AWAKE` / `SLEEP` / `STOP`。

## message

[message.py](../../mutsuki/contracts/message.py)
...
[event.py](../../mutsuki/contracts/event.py)
...
[capability.py](../../mutsuki/contracts/capability.py)
...
[capability_builtin.py](../../mutsuki/contracts/capability_builtin.py)

`Caps.READ_MESSAGE` / `SEND_MESSAGE` / `CALL_LLM` / `PERSIST` / `NETWORK_EGRESS` / `SPAWN_AGENT` / `HOLD_REF` / `BORROW_REF` / `PRODUCE_REF_STREAM`，由 `bootstrap_facade` 注册到 `mutsuki.core` owner。

## permission

[permission.py](../../mutsuki/contracts/permission.py)
...
[permission_builtin.py](../../mutsuki/contracts/permission_builtin.py)

`Perms.PUBLIC` / `Perms.AGENT_OWNER`。

## error

[error.py](../../mutsuki/contracts/error.py)
...
[refpayload.py](../../mutsuki/contracts/refpayload.py)
...
[plugin.py](../../mutsuki/contracts/plugin.py)
...
[service.py](../../mutsuki/contracts/service.py)
...
[schema.py](../../mutsuki/contracts/schema.py)
...
[decision.py](../../mutsuki/contracts/decision.py)
