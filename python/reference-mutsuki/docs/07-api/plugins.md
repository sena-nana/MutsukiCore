# API · `mutsuki.plugins`（Python reference）

`python/reference-mutsuki` 内置参考插件。当前 Rust 主链不依赖这些 Python 插件；
...
[__init__.py](../../mutsuki/plugins/echo/__init__.py)
...
通过 `pyproject.toml` 的 `[project.entry-points."mutsuki.plugins"]` 注册：

```toml
[project.entry-points."mutsuki.plugins"]
echo = "mutsuki.plugins.echo:EchoPlugin"
```

## echo.smoke

[smoke.py](../../mutsuki/plugins/echo/smoke.py)
...
完整流程：构造 Python reference Agent + JsonlTraceWriter + 装载 EchoPlugin + 启动 Python reference scheduler + 投 "echo hello" + 收响应 + 卸载。trace 落到 `<gettempdir()>/mutsuki-echo-smoke.jsonl`。

`smoke.py` 同时是 v0.1 门控的运行检查（参见 [v0.1 报告](../../plans/version-reports/v0.1.md)）。

## inmemory_endpoint

[__init__.py](../../mutsuki/plugins/inmemory_endpoint/__init__.py)

```python
class InMemoryEndpointPlugin(Plugin[_InMemoryConfig]):
    id = "mutsuki-inmemory-endpoint"
    version = "0.2.0"
    capabilities = [
        Capability(name=Caps.READ_MESSAGE),
        Capability(name=Caps.SEND_MESSAGE),
...
[__init__.py](../../mutsuki/plugins/onebot_v11/__init__.py)

```python
class OneBotV11Plugin(Plugin[_OneBotV11Config]):
    id = "mutsuki-onebot-v11"
    version = "0.2.0"
    capabilities = [
        Capability(name=Caps.READ_MESSAGE),
        Capability(name=Caps.SEND_MESSAGE),
