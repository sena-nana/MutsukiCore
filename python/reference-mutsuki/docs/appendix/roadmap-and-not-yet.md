...
当前 Mutsuki 是**单进程**。`ServiceContainer` 解析的是进程内对象；`Bus` 派发只在进程内；Agent 与 plugin 实例都在同一 event loop。
...
Mutsuki 核心**不内置 LLM 调用**（hard rule #2）。LLM 必须是插件，而且是桥接插件 —— 包装外部 SDK，通过 `Caps.CALL_LLM` 声明，via service 提供给业务命令。
...
无内置调试面板。trace 已有 `JsonlTraceWriter` / `JsonlTraceReader` 的 JSONL 读写闭环，`mutsuki.testing.replay_trace_spans(...)` 可校验重复 span、父链、时间区间与确定性排序；v0.4 的 `mutsuki.testing.contract_kit` 还提供 `assert_trace_tree_closed`、`assert_cross_agent_trace_chain`、`assert_dispatcher_clean`。可视化 UI 仍在后续版本。
...
[plans/architecture.md](../../plans/architecture.md) 提到 Mutsuki 是给 Yume / mind-sim 做运行核 —— 但 Yume 自身的实现路径将被解构为 Mutsuki 之上的零散插件。
