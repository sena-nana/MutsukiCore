...
if TYPE_CHECKING:
    from mutsuki.core.scope import PluginScope, TransactionScope
...
class RefDescriptor(Contract):
    ...
    schema_id: ClassVar[str] = "mutsuki.ref_descriptor"
...
class Handle(ABC, Generic[T]):
    ...
    具体实现见 :mod:`mutsuki.core.handle`。子类通过 ``__init_subclass__``
...
class RefPayload(Contract, Generic[T]):
    ...
    * 或者把 handle 替换为 descriptor（仅在显式降级用于 trace/audit 时，
    ...
    schema_id: ClassVar[str] = "mutsuki.ref_payload"
    schema_version: ClassVar[str] = "1.0.0"
