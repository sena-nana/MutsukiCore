...
from mutsuki.contracts.capability import Capability, CapabilityName
from mutsuki.contracts.error import Error, Errs
from mutsuki.contracts.operation import OperationDep, OperationDescriptor
from mutsuki.contracts.permission import PermissionName, PermissionRule
...
from mutsuki.contracts.source import SourceDep, SourceDescriptor
from mutsuki.core.dependency import (
    Dependent,
...
def command(
    *,
    ...
    Text command routing now lives in :mod:`mutsuki_ext.command`; core only
    declares Operations.
    """
...
def _build_command_spec(
    *,
    ...
    for pname, sig_param in sig.parameters.items():
        ...
        from mutsuki.contracts.plugin import Inject as _Inject
        from mutsuki.core.context import AgentContext as _Ctx
...
class Plugin(ABC, Generic[C], metaclass=PluginMeta):
    ...
    * ``capabilities: ClassVar[list[Capability]]``。
    ...
    路由由 ``mutsuki_ext.command`` 这类扩展提供。
    """
    ...
    async def on_envelope(self, envelope: "Envelope") -> None:
        ...
        envelope 类型为 :class:`mutsuki.contracts.envelope.Envelope` 基类；
