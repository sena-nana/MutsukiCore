...
import mutsuki.contracts as contracts
...
def test_construct_message_inherits_envelope_fields() -> None:
    ...
    assert m.payload_schema_id == "mutsuki.message"
...
def test_envelope_subclass_schemas_registered() -> None:
    ...
    assert SchemaRegistry.get("mutsuki.envelope") is Envelope
    assert SchemaRegistry.get("mutsuki.message") is Message
    assert SchemaRegistry.get("mutsuki.source_ref") is SourceRef
    assert SchemaRegistry.get("mutsuki.channel_ref") is ChannelRef
