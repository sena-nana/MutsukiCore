...
from mutsuki.contracts.schema import is_compatible, register_schema_compatibility


def test_default_compat_requires_byte_equal_versions() -> None:
    assert is_compatible("mutsuki.test.unregistered", "1.0.0", "1.0.0")
    assert not is_compatible("mutsuki.test.unregistered", "1.0.0", "1.0.1")


def test_register_callback_overrides_default() -> None:
    schema_id = "mutsuki.test.compat-register"
...
def test_register_overwrites_previous_callback() -> None:
    schema_id = "mutsuki.test.compat-overwrite"
