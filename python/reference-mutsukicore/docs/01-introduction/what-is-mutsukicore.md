...
**MutsukiCore 当前根级实现是 Rust-first Agent runtime framework；本目录记录旧 Python framework / plugin host 的 reference 形态。**

—— 摘自 [README.md](../../README.md)。

旧 Python framework 已移动到 `python/reference-mutsukicore/`，用于迁移、对照和复用
...
但 MutsukiCore 不是只服务 Yume。任何"消息进来 → 命令路由 → 调插件 → 回响应"的传统 bot 形态，都能用 EchoPlugin 那种插件写法实现。Agent 与 Plugin 解耦：核心域中立，业务全在插件里。
...
```text
Application / Host
  -> mutsukicore-runtime-host (optional native helper)
  -> mutsukicore-runtime-core
...
- 想要 NoneBot 那种成熟生态、上百个 adapter 现成可用的人 —— MutsukiCore 现在走的是 transport plugin 形态，不是独立 adapter 生态
- 想跑大规模分布式 bot 集群的人 —— v0.1 是单进程内
- 不想写类型注解的人 —— MutsukiCore 的 PluginMeta / Dependent 完全依赖类型注解工作
