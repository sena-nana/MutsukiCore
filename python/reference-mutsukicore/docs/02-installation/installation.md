...
```bash
git clone <repo-url> MutsukiCore
cd MutsukiCore
uv sync --all-extras
```
...
预期输出（来自 [smoke.py](../../mutsukicore/plugins/echo/smoke.py)）：
...
[smoke] phase=stop; trace at /tmp/mutsukicore-echo-smoke.jsonl
```
...
pip install -e ".[dev]"
pytest tests/
python -m mutsukicore.plugins.echo.smoke
```

`-e .` 是 editable install —— 你在 `mutsukicore/` 下改代码立刻生效，不用重装。
...
**Q: pip 报 `ERROR: Package 'mutsukicore' requires a different Python: 3.12.x not in '>=3.13'`**

A: MutsukiCore 锁 Python 3.13+。装一个 3.13：
...
**Q: `mutsukicore.plugins.echo.smoke` 报 `EntryPoint not found`**
