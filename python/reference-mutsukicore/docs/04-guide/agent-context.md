...
代码：[mutsukicore/core/context.py](../../mutsukicore/core/context.py)。
...
[context.py:32-46](../../mutsukicore/core/context.py#L32-L46)：
...
`trace_id` 跨整个外部触发链路保持不变，`span_id` 标识当前这一跳，`parent_span_id` 指向调用方的 span。文本命令入口由 [`TextCommandRouterPlugin`](../../mutsukicore_ext/command/__init__.py) 构造命令调用 ctx；generic envelope consumer 由 [`dispatch_envelope_to_consumers`](../../mutsukicore/runtime/envelope_dispatch.py) 构造 consumer ctx。插件需要嵌套调用时应当继承当前 ctx 的 `trace_id`、把当前 `span_id` 作为子调用的 `parent_span_id`。
...
```python
from mutsukicore import AgentContext, Plugin, command
