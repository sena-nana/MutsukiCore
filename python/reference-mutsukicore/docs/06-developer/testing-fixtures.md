...
MutsukiCore 内置一组用于测试的夹具：`ManualClock` / `DeterministicIdGen` / `SeededRng` / `make_stub_handle` / `InMemoryEndpointPlugin`，以及 v0.2 新增的 dispatcher contract helper、hard rule #14 lint 与 dispatcher invoke benchmark helper。
...
```python
from mutsukicore.runtime import ManualClock
...
```python
from mutsukicore.runtime import DeterministicIdGen
...
```python
from mutsukicore.runtime import SeededRng
...
这个模板测试 `python/reference-mutsukicore` 的旧插件链路。当前 Rust 主链的 native
smoke 覆盖在 `mutsukicore-runtime-host`，根级验证入口是 `cargo test`。
...
```python
from mutsukicore.testing.plugin_lint import lint_plugin_io_fields

violations = lint_plugin_io_fields("mutsukicore/plugins/onebot_v11/__init__.py")
assert violations == []
```
...
```python
from mutsukicore.testing.benchmark import measure_dispatcher_invoke
