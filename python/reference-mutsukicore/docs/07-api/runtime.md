# API · `mutsukicore.runtime`（Python reference）

本页描述 `python/reference-mutsukicore` 中旧 Python reference runtime 的 API：
...
来源：[mutsukicore/runtime/__init__.py](../../mutsukicore/runtime/__init__.py)。
...
[clock.py](../../mutsukicore/runtime/clock.py)
...
[idgen.py](../../mutsukicore/runtime/idgen.py)
...
[rng.py](../../mutsukicore/runtime/rng.py)
...
[scheduler.py](../../mutsukicore/runtime/scheduler.py)
...
`mutsukicore-runtime-host` 提供 native in-memory host helper，可在不依赖 Python 的情况下
...
[backend.py](../../mutsukicore/runtime/backend.py)
...
- `crates/mutsukicore-runtime-contracts`：纯协议与 `ScopeRuleSpec.matches(...)`。
- `crates/mutsukicore-runtime-core`：`AgentRuntime`、backend trait、Operation registry、
