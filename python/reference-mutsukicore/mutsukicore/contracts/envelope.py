...
基本单位；IM ``Message`` 是其特化（参见 :mod:`mutsukicore.contracts.message`）。
其他外部后端或领域输入应由 bridge / 领域插件声明自己的 Envelope 子类与
``SourceKindName``。

路由按 ``payload_schema_id`` 与 ``source.source_id`` / ``source.kind`` /
``capabilities_required`` 决定（参见 :mod:`mutsukicore.contracts.scope`）。
"""
...
from mutsukicore.contracts.ids import EnvelopeId
...
class SourceRef(Contract):
    ...
    schema_id: ClassVar[str] = "mutsukicore.source_ref"
...
class Envelope(Contract):
    ...
    schema_id: ClassVar[str] = "mutsukicore.envelope"
    schema_version: ClassVar[str] = "1.0.0"
