...
import mutsukicore.contracts as contracts
...
def test_construct_message_inherits_envelope_fields() -> None:
    ...
    assert m.payload_schema_id == "mutsukicore.message"
...
def test_envelope_subclass_schemas_registered() -> None:
    ...
    assert SchemaRegistry.get("mutsukicore.envelope") is Envelope
    assert SchemaRegistry.get("mutsukicore.message") is Message
    assert SchemaRegistry.get("mutsukicore.source_ref") is SourceRef
    assert SchemaRegistry.get("mutsukicore.channel_ref") is ChannelRef
