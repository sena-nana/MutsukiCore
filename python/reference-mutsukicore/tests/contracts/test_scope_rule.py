...
def _msg(
    *,
    schema: str = "mutsukicore.message",
    source_id: str = "im:1",
    kind=SourceKinds.IM,
    caps=(),
    channel: str = "c",
...
def test_by_schema_matches_exact() -> None:
    rule = BySchema("mutsukicore.message")
    assert rule.check(_msg(schema="mutsukicore.message"))
...
def test_by_schema_prefix_matches() -> None:
    ...
    assert not rule.check(_msg(schema="mutsukicore.message"))
...
def test_and_requires_both() -> None:
    rule = BySchema("mutsukicore.message") & BySourceKind(SourceKinds.IM)
    assert rule.check(_msg(schema="mutsukicore.message", kind=SourceKinds.IM))
    assert not rule.check(_msg(schema="other", kind=SourceKinds.IM))
    assert not rule.check(_msg(schema="mutsukicore.message", kind=BackendKind))
...
def test_scopes_im_text_matches_im_text_message() -> None:
    ...
    assert rule.check(
        _msg(schema="mutsukicore.message", kind=SourceKinds.IM, caps=(Caps.IM_TEXT,))
    )
...
def test_scopes_im_text_rejects_non_im() -> None:
    ...
    assert not rule.check(
        _msg(schema="mutsukicore.message", kind=BackendKind, caps=(Caps.IM_TEXT,))
    )
...
def test_scope_name_register_rejects_unknown_construction() -> None:
    ...
    from mutsukicore.contracts.scope import UnknownScopeError
